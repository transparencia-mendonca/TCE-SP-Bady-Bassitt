...
const DBN="tcesp-bady-bassitt-v6",STORE="dados",KEY="base";
...
function tceExpenseUrl(year){return `https://transparencia.tce.sp.gov.br/municipio/bady-bassitt/${year}/despesas`}
...
function auditExcludedSupplier(row){let t=normalizedName(row);return /FOLHA DE PAGAMENTO|PAGAMENTOS[- ]SERVIDORES|ENERGISA|TELEFONICA|TELECOM|CORREIOS|TRIVALE|ROM CARD|INSS|INSTITUTO NACIONAL DO SEGURO SOCIAL|CAIXA ECONOMICA|BANCO DO BRASIL|CAMARA MUNICIPAL|PREFEITURA MUNICIPAL|MUNICIPIO DE BADY BASSITT/.test(t)}
...
const MUNICIPIO_API="bady-bassitt";
...
(async()=>{try{
  db=await idbGet();
  if(!db){
    let old=localStorage.getItem("tcesp-bady-bassitt-data")||localStorage.getItem("tcesp-data");
    if(old){
      db=JSON.parse(old);
    }else{
  ...
  await save();
  localStorage.removeItem("tcesp-bady-bassitt-data");
}catch(e){
